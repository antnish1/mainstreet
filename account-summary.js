...
const sessionToken=()=>JSON.parse(localStorage.getItem('mainstreet.staff-session.v3')||'null')?.token||null;
const money=n=>`₹${Number(n||0).toLocaleString('en-IN',{minimumFractionDigits:2,maximumFractionDigits:2})}`;
const safe=v=>String(v||'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;');
async function rpc(name,body){const response=await fetch(`${SUPABASE_URL}/rest/v1/rpc/${name}`,{method:'POST',headers,body:JSON.stringify(body)});const text=await response.text();let data;try{data=text?JSON.parse(text):null}catch{data=text}if(!response.ok)throw Error(data?.message||data?.hint||'Request failed');return data}
...
function openCustomerLedger(name,phone){const end=new Date();const start=new Date();start.setDate(end.getDate()-29);$('#ledgerCustomer').value=name;$('#ledgerPhone').value=phone||'';$('#ledgerFrom').value=dateValue(start);$('#ledgerTo').value=dateValue(end);$('#generateLedger').click();setTimeout(()=>$('#ledgerList')?.scrollIntoView({behavior:'smooth',block:'start'}),250)}

$('#openExpense')?.addEventListener('click',()=>setTimeout(setupExpenseLedger,80));
$('#openLedger')?.addEventListener('click',()=>setTimeout(setupCustomerSummary,80));