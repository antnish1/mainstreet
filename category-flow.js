...
function updateSelectionCounters(){const counts=invoiceCounts();menuList?.querySelectorAll('.menu-item').forEach(row=>{const name=row.querySelector(':scope > div:first-child b')?.textContent?.trim();const count=counts.get(name)||{half:'0',full:'0'};const half=row.querySelector('.menu-item-actions .half'),full=row.querySelector('.menu-item-actions .full');if(half){half.dataset.count=count.half;half.classList.toggle('has-count',Number(count.half)>0)}if(full){full.dataset.count=count.full;full.classList.toggle('has-count',Number(count.full)>0)}});if(itemDialogCount&&selectedCount)itemDialogCount.textContent=selectedCount.textContent;if(itemDialogTotal&&menuTotal)itemDialogTotal.textContent=menuTotal.textContent}
const observer=new MutationObserver(()=>requestAnimationFrame(updateSelectionCounters));if(menuList)observer.observe(menuList,{childList:true,subtree:true,characterData:true,attributes:true});if(invoiceItems)observer.observe(invoiceItems,{childList:true,subtree:true,characterData:true});if(menuTotal)observer.observe(menuTotal,{childList:true,subtree:true,characterData:true});updateSelectionCounters();